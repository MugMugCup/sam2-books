...
def extract_frames_to_dir(
    ...
    print(video_path, out_dir, max_long_side)
    while True:
        ret, frame = cap.read()
        if not ret:
            break
...
def get_mem_stats():
  """
  返り値: dict
    rss_mb         … OS視点の常駐セットサイズ
    vms_mb         … 仮想メモリサイズ
    py_current_mb  … tracemallocで追跡中の現在値
    py_peak_mb     … tracemallocのピーク
    cuda_alloc_mb  … CUDAで実際に確保中の量
    cuda_reserved_mb … CUDAで予約中の量
  """
  import os
  import math
  import torch
  ...
  return {
    "rss_mb": rss / MB,
    "vms_mb": vms / MB,
    "py_current_mb": py_cur / MB,
    "py_peak_mb": py_peak / MB,
    "cuda_alloc_mb": cuda_alloc / MB,
    "cuda_reserved_mb": cuda_reserved / MB,
  }

def format_mem_line(stage, stats):
  return (
    f"{stage} | RSS: {stats['rss_mb']:.1f} MB, "
    f"VMS: {stats['vms_mb']:.1f} MB, "
    f"Py(cur/peak): {stats['py_current_mb']:.1f}/{stats['py_peak_mb']:.1f} MB, "
    f"CUDA(alloc/resv): {stats['cuda_alloc_mb']:.1f}/{stats['cuda_reserved_mb']:.1f} MB"
  )
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    # メモリの占有率を調べるために追加
    def _init_memlog(self):
        # CSVのファイル名を自動作成（例: memlog_20250905_120000.csv）
        ts = _now_str()
        self._memlog_path = f"memlog_{ts}.csv"
        try:
            with open(self._memlog_path, "w", encoding="utf-8") as f:
                f.write("timestamp,stage,rss_mb,vms_mb,py_current_mb,py_peak_mb,cuda_alloc_mb,cuda_reserved_mb\n")
        except Exception as e:
            print(f"[memlog] init failed: {e}")

    def _log_mem(self, stage: str):
        stats = get_mem_stats()
        # コンソール出力
        print(format_mem_line(stage, stats))
        # CSV追記
        try:
            import time
            with open(self._memlog_path, "a", encoding="utf-8") as f:
                f.write(
                f"{time.strftime('%Y-%m-%d %H:%M:%S')},"
                f"{stage},"
                f"{stats['rss_mb']:.3f},"
                f"{stats['vms_mb']:.3f},"
                f"{stats['py_current_mb']:.3f},"
                f"{stats['py_peak_mb']:.3f},"
                f"{stats['cuda_alloc_mb']:.3f},"
                f"{stats['cuda_reserved_mb']:.3f}\n"
                )
        except Exception as e:
            print(f"[memlog] write failed: {e}")
    ...
    def load_video(self, path: str):
        ...
        # ← ここで一度（フレーム展開直後）
        self._log_mem("after_extract_frames")
        ...
        # ← ここで二度目（SAM2初期化直後）
        self._log_mem("after_init_state")
